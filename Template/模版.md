...
<img src="/Users/dave/Desktop/codebase/Template/LGV 引理1.png" alt="LGV 引理1" style="zoom:40%;" />

<img src="/Users/dave/Desktop/codebase/Template/LGV 引理2.png" alt="LGV 引理2" style="zoom:40%;" />
...
一个数有原根当且仅当该数为 $2, 4, p^{k}, 2p^{k}$，其中 $p$ 为奇素数
```c++
...
int phi(int x)
{
	vector<int> fac=divide(x);
	int num=x;
	for(int e: fac) num=num/e*(e-1);
	return num;
}
...
{
	int tmp=phi(m);
    vector<int> fac=divide(tmp);
    for(int i=2; i<m; ++i) {
        if(__gcd(i, m)!=1) continue;
        int flag=1;
        for(int e: fac)
            if(e!=tmp && qpow(i, tmp/e, m)==1) {
                flag=0;
                break;
            }
        if(flag) return i;
...
给定集合幂级数 $a, b$, 求 $c_i = \sum\limits_{j \cup k = i, j\cap = \emptyset} a_{j} b_{k}$

```c++
int n, m, cnt[MAXN];
vector<int> a[21], b[21], c[21];
...
int main()
{
    ...
    for(int i=0; i<m; ++i) scanf("%d", &b[cnt[i]][i]);
    for(int i=0; i<=n; ++i) {
        or_fwt(a[i], 1);
        or_fwt(b[i], 1);
    }
...
代价函数满足四边形不等式当且仅当对于任何 $p_1\leq p_2 \leq p_3 \leq p_4$ 满足 $c(p1, p4)+c(p2, p3) \geq c(p1, p3)+c(p2,p4)$

代价函数满足区间包含性当且仅当对于任何 $p_1\leq p_2 \leq p_3 \leq p_4$ 满足 $c(p1, p4) \geq c(p2,p3)$

**决策单调性**

一维问题：转移方程形如 $f_i=\min\{ f_{j}+c(j, i) \}$, 若代价满足四边形不等式，则转移满足决策单调性

```c++
// p 为决策点，l r 表示从 p 转移到的区间
struct Sec {int p, l, r;} q[MAXN];
...
int find(int x)
{
    int l=max(q[tail].l, x+1), r=n+1;
    int y=q[tail].p, ans=r+1;
    while(l<=r) {
        int mid=(l+r)/2;
        if(cal(x, mid)<=cal(y, mid)) ans=mid, r=mid-1;
        else l=mid+1;
    }
    return ans;
}
...
void solve()
{
    head=tail=1; q[tail]={0, 1, n};
    for(int i=1; i<=n; ++i) {
        while(head<tail && q[head].r<i) head++;
        f[i]=cal(q[head].p, i); g[i]=q[head].p;
        
        int p=find(i);
        while(tail>head && p<=q[tail].l) {
            tail--;
            p=find(i);
        }
        if(p!=n+1) {
...
二维问题：转移方程形如 $f_{i, j}=\min\{ f_{i-1, j}+c(j, i) \}$, 设 $f_{i,j}$ 的最优决策点为 $pos_{i,j}$，若代价满足四边形不等式和区间包含性，则有 $pos_{i-1, j} \leq pos_{i, j} \leq pos_{i, j+1}$

```c++
int n, k, v[MAXN][MAXN], f[MAXN][MAXN], pos[MAXN][MAXN];

int val(int x, int y)
{
    return v[y][y]-v[x][y]-v[y][x]+v[x][x];
}
...
int main()
{
    ...
    for(int i=1; i<=n; ++i) {
        ...
        for(int j=1; j<=n; ++j) v[i][j]+=v[i-1][j];
    }
    ...
    for(int i=1; i<=k; ++i) {
        pos[i][n+1]=n;
        for(int j=n; j>=i; --j) {
            for(int k=pos[i-1][j]; k<=min(j-1, pos[i][j+1]); ++k) {
                if(f[i-1][k]+val(k, j)<f[i][j]) {
                    f[i][j]=f[i-1][k]+val(k, j);
                    pos[i][j]=k;
                }
            }
        }
...
转移方程形如 $f_i=a_i \cdot b_j+c_i+d_j$ 时，可以使用斜率优化
```c++
int n,L;
